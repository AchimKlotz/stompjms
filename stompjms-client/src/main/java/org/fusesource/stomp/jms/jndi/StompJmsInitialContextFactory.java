...
/**
 * A factory of the StompJms InitialContext which contains {@link javax.jms.ConnectionFactory} instances as well as a
 * child context called <i>destinations</i> which contain all of the current active destinations, in child context
...
public class StompJmsInitialContextFactory implements InitialContextFactory {
    ...
    private static final String[] DEFAULT_CONNECTION_FACTORY_NAMES = { "ConnectionFactory", "QueueConnectionFactory",
    ...
    public Context getInitialContext(Hashtable<?, ?> environment) throws NamingException {
        ...
        Hashtable<String, Object> env = (Hashtable<String, Object>) environment;
        ...
        String[] names = getConnectionFactoryNames(env);
        for (int i = 0; i < names.length; i++) {
            String name = names[i];

            try {
                StompJmsConnectionFactory factory = createConnectionFactory(name, env);
                /*
                 * if( broker==null ) { try { broker = factory.getEmbeddedBroker(); } catch (JMSException e) { log.warn(
                 * "Failed to get embedded broker", e); } }
                 */
    ...
    protected ReadOnlyContext createContext(Hashtable<String, Object> environment, Map<String, Object> data) {
        return new ReadOnlyContext(environment, data);
    }

    protected StompJmsConnectionFactory createConnectionFactory(String name, Hashtable<String, Object> environment) {
        Hashtable<String, Object> temp = new Hashtable<>(environment);
    ...
    protected String[] getConnectionFactoryNames(Map<String, Object> environment) {
     ...
     */
    protected StompJmsConnectionFactory createConnectionFactory(Hashtable<String, Object> environment) {
